...
* source: <https://github.com/Sarv/email-spam-scan>
* package: `@sarv-in/email-spam-scan`
...
| --- | --- |
| `@sarv-in/email-spam-scan` | Everything. Main process / core only. |
| `@sarv-in/email-spam-scan/headers` | Reading raw header text: `headerLookupFromText`, `bulkHeaderSignals`, the auth-header reader. Zero dependencies. |
| `@sarv-in/email-spam-scan/verdict` | Thresholds, `spamVerdict`, `parseSpamReasons`, `AuthStatus`. |
| `@sarv-in/email-spam-scan/identity` | `registrableDomain`, `domainsInText`, `assessSender`. |
| `@sarv-in/email-spam-scan/links` | `linkMismatches`, `assessLinks`, `assessPhishing`. |
| `@sarv-in/email-spam-scan/security` | `assessEmailSecurity` and the level model. |
| `@sarv-in/email-spam-scan/reputation` | The blocklist catalogue and the DNSBL lookup. `BlocklistsTab.tsx` imports it for the catalogue: it reaches `node:dns` only through a dynamic import, and its one third-party cost, `ipaddr.js`, is CommonJS and therefore named in `LINKED_CJS_DEPS` (see below). |
...
- **A circuit breaker.** This is the desktop-specific one. Spamhaus and its
  peers *refuse* queries that arrive through a public or open resolver — which
...
**Working on the library** (the current state): point at the sibling checkout.

```json
"@sarv-in/email-spam-scan": "file:../../../email-spam-scan"
```
...
**Releasing**: point at the registry and empty the list.

```json
"@sarv-in/email-spam-scan": "^0.1.0"
```
...
startup time. `@sarv-in/email-chat-view` is the standing example of a sibling
library in this second state.
